<?php
require_once('../config.php'); // This line is for config/constants, not for loading $_settings
                               // This file should define the DB_SERVER, DB_USERNAME, etc. constants.
                               // It should NOT instantiate $_settings.

// This file (Master.php) should be included by your main initialization script.

// Ensure DBConnection class is defined/included BEFORE Master class
// If your initialize.php handles this, then it's fine. Otherwise, you'd add:
// require_once('DBConnection.php'); // Assuming DBConnection.php is in the same directory

Class Master extends DBConnection {
    // REMOVE THIS LINE: private $settings; // This is the source of the problem

    // Add a property to store user session data directly within Master
    private $user_data = [];
    ...
    public function __construct(){
        ...
        parent::__construct(); // Call DBConnection's constructor to establish $this->conn

        // Ensure session is started for session-related operations
        if (session_status() === PHP_SESSION_NONE) {
            session_start();
        }
        // Load user data when the Master object is created
        $this->load_user_data();
    }
    ...
    // New method to load user data from session
    private function load_user_data() {
        if (isset($_SESSION['userdata'])) {
            $this->user_data = $_SESSION['userdata'];
        } else {
            // Initialize with default/null values to prevent "Undefined index" errors
            $this->user_data = [
                'id' => null,
                'type' => null,
                'firstname' => '',
                'lastname' => '',
                // ... add other expected keys with default null/empty values
            ];
        }
    }

    // Implement the userdata method directly in the Master class
    ...
    // Implement set_flashdata method directly in the Master class
    ...
    public function get_flashdata($key) {
        if (isset($_SESSION['flashdata'][$key])) {
            ...
            unset($_SESSION['flashdata'][$key]); // Clear it after reading
            return $value;
        }
        return null;
    }
    ...
    function save_source(){
        ...
        if($resp['status'] =='success')
            // Change: $this->settings->set_flashdata to $this->set_flashdata
    ...
    function delete_source(){
        ...
        if($del){
            $resp['status'] = 'success';
            // Change: $this->settings->set_flashdata to $this->set_flashdata
            $this->set_flashdata('success'," Source has been deleted successfully.");
        }else{
            $resp['status'] = 'failed';
            $resp['error'] = $this->conn->error;
        }
    ...
    public function save_lead() {
        ...
        $is_done = 0;
        // Change: $_SESSION['userdata']['id'] to $this->userdata('id')
        ...
        if (!empty($id)) {
            ...
            $sql = "INSERT INTO lead_list (code, source_id, interested_in, remarks, assigned_to, user_id, status, lead_type, project_name, other_info, is_done, date_created, date_updated)
                VALUES (
                    '$code',
                    '$source_id',
                    '$interested_in',
    ...
    function delete_lead(){
        ...
        if($del){
            $resp['status'] = 'success';
            // Change: $this->settings->set_flashdata to $this->set_flashdata
            $this->set_flashdata('success'," Lead has been deleted successfully.");
        }else{
            $resp['status'] = 'failed';
            $resp['error'] = $this->conn->error;
        }
    ...
    function save_call_reminder() {
        ...
        // Change: $this->settings->userdata('id') to $this->userdata('id')
        $user_id = $this->userdata('id'); // logged-in agent
    ...
    function save_log(){
        if(empty($_POST['id'])){
            // Change: $this->settings->userdata('id') to $this->userdata('id')
            $_POST['user_id'] = $this->userdata('id');
        }
        extract($_POST);
    ...
    function delete_log(){
        ...
        if($del){
            $resp['status'] = 'success';
            // Change: $this->settings->set_flashdata to $this->set_flashdata
            $this->set_flashdata('success'," Log has been deleted successfully.");
        }else{
            $resp['status'] = 'failed';
            $resp['error'] = $this->conn->error;
        }
    ...
    function delete_note(){
        ...
        if($del){
            $resp['status'] = 'success';
            // Change: $this->settings->set_flashdata to $this->set_flashdata
            $this->set_flashdata('success'," Note has been deleted successfully.");
        }else{
            $resp['status'] = 'failed';
            $resp['error'] = $this->conn->error;
        }
...
// The following lines are usually in a separate file that includes Master.php and DBConnection.php
// This setup means this file is both defining the class and acting as a controller/API endpoint.

// If this entire block is your `master.php` file, then this is correct.
// However, the `require_once('../config.php');` at the top suggests `config.php` might be a
// separate file for constants only.
// The crucial step is how this `Master.php` is *included* in your `index.php` or other scripts.

// Instantiate the Master class
$Master = new Master();
// Assign the Master instance to the global $_settings variable for wider access.
// This is the line that was missing or incorrectly handled previously.
$_settings = $Master;

$action = !isset($_GET['f']) ? 'none' : strtolower($_GET['f']);
// $sysset = new SystemSettings(); // This line is commented out, so it's not relevant for this error
switch ($action) {
    case 'save_source':
