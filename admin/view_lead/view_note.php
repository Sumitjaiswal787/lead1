...
$users = $conn->query("SELECT id,CONCAT(firstname, ' ', lastname, ' ', COALESCE(middlename,'')) as fullname FROM `users` where id in (SELECT `user_id` FROM `note_list` where lead_id = '{$lead_id}')");
